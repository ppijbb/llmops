...
if 'target_text' not in st.session_state:
    st.session_state.target_text = input_language

if 'lecture_prompt' not in st.session_state:
    st.session_state.lecture_prompt = ""

if 'lecture_text' not in st.session_state:
    st.session_state.lecture_text = input_text
...
with input_col:
    ...
    st.title("LLM 강연 요약")
    with st.form("강연 요약 데모"):
        s1, s2 =  st.columns(2)
        with s1:
            st.text_area(
                    label="입력된 강연을 요약 프롬프트 입니다.", 
                    key="lecture_prompt",
                    height=400)
        with s2:
            st.text_area(
...
with output_col:
    ...
    st.title("강연 요약 결과")
    with st.container(border=True):
        st.markdown("llm 강연 요약 결과가 여기에 표시됩니다.")
        if summarize_button:
            with st.spinner('Wait for it...'):
                start = datetime.now()
                response = requests.post(
